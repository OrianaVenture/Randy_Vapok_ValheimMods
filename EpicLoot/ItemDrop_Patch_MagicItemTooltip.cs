...
namespace EpicLoot
{
    ...
    public static class InventoryGrid_CreateItemTooltip_MagicItemComponent_Patch
    {
        public static bool Prefix(ItemDrop.ItemData item, UITooltip tooltip)
        {
            ...
            if (item.IsEquipable() && !item.m_equipped && Player.m_localPlayer != null && Player.m_localPlayer.HasEquipmentOfType(item.m_shared.m_itemType) && Input.GetKey(KeyCode.LeftControl))
            {
                ...
                tooltipText = item.GetTooltip() + $"<color={EpicColors.LightGrey}><i>$mod_epicloot_currentlyequipped:</i></color>\n<size=18>{otherItem.GetDecoratedName()}</size>\n" + otherItem.GetTooltip();
            }
            else
            {
    ...
    public static class MagicItemTooltip_ItemDrop_Patch
    {
        ...
        private static bool Prefix(ref string __result, ItemDrop.ItemData item, int qualityLevel, bool crafting)
        {
            ...
            {
                text.Append($"\n<color={EpicColors.Aqua}>$item_dlc</color>");
            }

            ItemDrop.ItemData.AddHandedTip(item, text);
            if (item.m_crafterID != 0L)
            {
                text.AppendFormat($"\n$item_crafter: <color={EpicColors.Orange}>{0}</color>", item.GetCrafterName());
            }

            if (!item.m_shared.m_teleportable)
            {
                text.Append($"\n<color={EpicColors.Orange}>$item_noteleport</color>");
            }

            if (item.m_shared.m_value > 0)
            {
                text.AppendFormat($"\n$item_value: <color={EpicColors.Orange}>{0} ({1})</color>", item.GetValue(), item.m_shared.m_value);
            }

            var weightColor = magicItem.HasEffect(MagicEffectType.ReduceWeight) || magicItem.HasEffect(MagicEffectType.Weightless) ? magicColor : EpicColors.Orange;
            text.Append($"\n$item_weight: <color={weightColor}>{item.GetWeight():0.0}</color>");

            if (item.m_shared.m_maxQuality > 1)
            {
                text.AppendFormat($"\n$item_quality: <color={EpicColors.Orange}>{0}</color>", qualityLevel);
            }

            var indestructible = magicItem.HasEffect(MagicEffectType.Indestructible);
            if (!indestructible && item.m_shared.m_useDurability)
            {
                var maxDurabilityColor1 = magicItem.HasEffect(MagicEffectType.ModifyDurability) ? magicColor : EpicColors.Orange;
                var maxDurabilityColor2 = magicItem.HasEffect(MagicEffectType.ModifyDurability) ? magicColor : EpicColors.Yellow;
                ...
                if (item.m_shared.m_canBeReparied)
                {
                    ...
                    if (recipe != null)
                    {
                        ...
                        text.AppendFormat($"\n$item_repairlevel: <color={EpicColors.Orange}>{0}</color>", minStationLevel.ToString());
                    }
                }
            }
            ...
            var magicBlockColor1 = magicBlockPower ? magicColor : EpicColors.Orange;
            var magicBlockColor2 = magicBlockPower ? magicColor : EpicColors.Yellow;
            ...
            var magicParryColor = magicParry ? magicColor : EpicColors.Orange;
            switch (item.m_shared.m_itemType)
            {
                case ItemDrop.ItemData.ItemType.Consumable:
                    if (item.m_shared.m_food > 0.0)
                    {
                        text.AppendFormat($"\n$item_food_health: <color={EpicColors.Orange}>{0}</color>", item.m_shared.m_food);
                        text.AppendFormat($"\n$item_food_stamina: <color={EpicColors.Orange}>{0}</color>", item.m_shared.m_foodStamina);
                        text.AppendFormat($"\n$item_food_duration: <color={EpicColors.Orange}>{0}s</color>", item.m_shared.m_foodBurnTime);
                        text.AppendFormat($"\n$item_food_regen: <color={EpicColors.Orange}>{0} hp/tick</color>", item.m_shared.m_foodRegen);
                    }
                ...
                case ItemDrop.ItemData.ItemType.Torch:
                    ...
                    var magicAttackStaminaColor = magicAttackStamina ? magicColor : EpicColors.Orange;
                    ...
                    if (item.m_shared.m_attack.m_attackEitr > 0.0)
                        text.Append($"\n$item_eitruse: <color={EpicColors.Orange}>{item.m_shared.m_attack.m_attackEitr}</color>");
                    if (item.m_shared.m_attack.m_attackHealth > 0.0)
                        text.Append($"\n$item_healthuse: <color={EpicColors.Orange}>{item.m_shared.m_attack.m_attackHealth}</color>");
                    if (item.m_shared.m_attack.m_attackHealthPercentage > 0.0)
                        text.Append($"\n$item_healthuse: <color={EpicColors.Orange}>{item.m_shared.m_attack.m_attackHealthPercentage:0.0}%</color>");
                    if (item.m_shared.m_attack.m_drawStaminaDrain > 0.0)
                        text.Append($"\n$item_staminahold: <color={EpicColors.Orange}>{item.m_shared.m_attack.m_drawStaminaDrain}</color>/s");
                    ...
                    text.AppendFormat($"\n$item_knockback: <color={EpicColors.Orange}>{0}</color>", item.m_shared.m_attackForce);
                    ...
                    var magicBackstabColor = magicBackstab ? magicColor : EpicColors.Orange;
                ...
                case ItemDrop.ItemData.ItemType.Shoulder:
                    var magicArmorColor = magicItem.HasEffect(MagicEffectType.ModifyArmor) ? magicColor : EpicColors.Orange;
            ...
            if ((magicEitrRegen || item.m_shared.m_eitrRegenModifier != 0) && localPlayer != null)
            {
                ...
                var color = (magicEitrRegen) ? magicColor : EpicColors.Orange;
                var totalColor = equipMagicEitrRegenModifier > 0 ? magicColor : EpicColors.Yellow;
            ...
            if ((magicMovement || item.m_shared.m_movementModifier != 0) && localPlayer != null)
            {
                ...
                var color = (removePenalty || magicMovement) ? magicColor : EpicColors.Orange;
        ...
        public static void Postfix(ref string __result, ItemDrop.ItemData item)
        {
            ...
            if (!item.IsMagic())
            {
                ...
                if (item.IsSetItem())
                {
                    // TODO: add support for Therzie's modded item sets to appear in tooltip
            ...
            // TODO: Why is this happening? Does all the orange and yellow need to be replaced?
            __result = __result.Replace($"<color={EpicColors.Orange}>", $"<color={EpicColors.LightBlue}>");
            __result = __result.Replace($"<color={EpicColors.Yellow}>", $"<color={EpicColors.LightBlue}>");
        ...
        public static string DamageRange(float damage, float minFactor, float maxFactor, bool magic = false, string magicColor = "")
        {
            ...
            var color1 = magic ? magicColor : EpicColors.Orange;
            var color2 = magic ? magicColor : EpicColors.Yellow;
        ...
        public static Tuple<string, string> PreprocessTooltipStat(ItemDrop.ItemData item, string label, string value)
        {
            ...
            if (item.IsMagic(out var magicItem))
            {
                ...
                if (label.StartsWith("$item_movement_modifier") && (magicItem.HasEffect(MagicEffectType.RemoveSpeedPenalty) || magicItem.HasEffect(MagicEffectType.ModifyMovementSpeed)))
                {
                    ...
                    var endColorIndex = label.IndexOf(">", StringComparison.Ordinal);
                    if (colorIndex >= 0 && endColorIndex >= 0)
                    {
                        // TODO: check this string length matcher works with new colors
                        var sb = new StringBuilder(label);
                        sb.Remove(colorIndex, endColorIndex - colorIndex);
                        ...
                        var valueIndex = colorIndex + (endColorIndex - colorIndex);
