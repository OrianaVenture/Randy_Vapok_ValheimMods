            ...
            private static void Postfix(CharacterDrop __instance) {
                if (Player.m_localPlayer != null && Player.m_localPlayer.HasActiveMagicEffect(MagicEffectType.HeadHunter, out float effectValue, 0.01f)) {
                    ...
                    if (addTrophy) {
                        ...
                        GameObject go = GameObject.Instantiate(trophyGo, (__instance.transform.position + Vector3.up * 0.5f), Quaternion.Euler(0f, UnityEngine.Random.Range(0, 360), 0f));
